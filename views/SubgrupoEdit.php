...
loadjs.ready("head", function () {
    ...
    fsubgrupoedit = currentForm = new ew.Form("fsubgrupoedit", "edit");
    ...
    // Validate form
    fsubgrupoedit.validate = function () {
    ...
    // Form_CustomValidate
    fsubgrupoedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fsubgrupoedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fsubgrupoedit.lists.id_grupo = <?= $Page->id_grupo->toClientList($Page) ?>;
    loadjs.done("fsubgrupoedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fsubgrupoedit" id="fsubgrupoedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<?php if ($Page->id_subgrupo->Visible) { // id_subgrupo ?>
    <div id="r_id_subgrupo" class="form-group row">
        <label id="elh_subgrupo_id_subgrupo" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id_subgrupo->caption() ?><?= $Page->id_subgrupo->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id_subgrupo->cellAttributes() ?>>
<span id="el_subgrupo_id_subgrupo">
...
<input type="hidden" data-table="subgrupo" data-field="x_id_subgrupo" data-hidden="1" name="x_id_subgrupo" id="x_id_subgrupo" value="<?= HtmlEncode($Page->id_subgrupo->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<input type="hidden" name="fa_x_imagen_subgrupo" id= "fa_x_imagen_subgrupo" value="<?= (Post("fa_x_imagen_subgrupo") == "0") ? "0" : "1" ?>">
...
loadjs.ready("load", function () {
    // Startup script
    $("label#elh_subgrupo_nombre_subgrupo").remove(),$("label#elh_subgrupo_id_grupo").remove(),$("label#elh_subgrupo_descripcion_subgrupo").remove(),$("label#elh_subgrupo_imagen_subgrupo").remove(),$("label#elh_subgrupo_participante").remove();
});
</script>
