...
loadjs.ready("head", function () {
    ...
    fescenarioedit = currentForm = new ew.Form("fescenarioedit", "edit");
    ...
    // Validate form
    fescenarioedit.validate = function () {
    ...
    // Form_CustomValidate
    fescenarioedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fescenarioedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fescenarioedit.lists.tipo_evento = <?= $Page->tipo_evento->toClientList($Page) ?>;
    fescenarioedit.lists.incidente = <?= $Page->incidente->toClientList($Page) ?>;
    fescenarioedit.lists.pais_escenario = <?= $Page->pais_escenario->toClientList($Page) ?>;
    fescenarioedit.lists.estado = <?= $Page->estado->toClientList($Page) ?>;
    loadjs.done("fescenarioedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fescenarioedit" id="fescenarioedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<?php if ($Page->id_escenario->Visible) { // id_escenario ?>
    <div id="r_id_escenario" class="form-group row">
        <label id="elh_escenario_id_escenario" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id_escenario->caption() ?><?= $Page->id_escenario->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id_escenario->getDisplayValue($Page->id_escenario->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="escenario" data-field="x_id_escenario" data-hidden="1" name="x_id_escenario" id="x_id_escenario" value="<?= HtmlEncode($Page->id_escenario->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fescenarioedit", "editor"], function() {
	ew.createEditor("fescenarioedit", "x_descripcion_escenario", 20, 4, <?= $Page->descripcion_escenario->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
...
<script>
loadjs.ready(["fescenarioedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fescenarioedit", "x_fechaini_simulado", {"ignoreReadonly":true,"useCurrent":false,"format":109});
});
</script>
<?php } ?>
</span>
...
<script>
loadjs.ready(["fescenarioedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fescenarioedit", "x_fechafin_simulado", {"ignoreReadonly":true,"useCurrent":false,"format":109});
});
</script>
<?php } ?>
</span>
...
<script>
loadjs.ready(["fescenarioedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fescenarioedit", "x_fechaini_real", {"ignoreReadonly":true,"useCurrent":false,"format":109});
});
</script>
<?php } ?>
</span>
...
<script>
loadjs.ready(["fescenarioedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fescenarioedit", "x_fechafinal_real", {"ignoreReadonly":true,"useCurrent":false,"format":109});
});
</script>
<?php } ?>
</span>
...
<input type="hidden" name="fa_x_image_escenario" id= "fa_x_image_escenario" value="<?= (Post("fa_x_image_escenario") == "0") ? "0" : "1" ?>">
...
<?php if ($Page->estado->Visible) { // estado ?>
    <div id="r_estado" class="form-group row">
        <label id="elh_escenario_estado" class="<?= $Page->LeftColumnClass ?>"><?= $Page->estado->caption() ?><?= $Page->estado->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<?php
    if (in_array("grupo", explode(",", $Page->getCurrentDetailTable())) && $grupo->DetailEdit) {
?>
...
<?php } ?>
<?php
    if (in_array("tareas", explode(",", $Page->getCurrentDetailTable())) && $tareas->DetailEdit) {
?>
<?php if ($Page->getCurrentDetailTable() != "") { ?>
