 ...
 * Page class
 */
class MensajesGrid extends Mensajes
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MensajesGrid";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fmensajesgrid";
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->id_inyect->CurrentValue;
    ...
    public function emptyRow()
    {
        ...
        if ($CurrentForm->hasValue("x_mensaje") && $CurrentForm->hasValue("o_mensaje") && $this->mensaje->CurrentValue != $this->mensaje->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_fechareal_start") && $CurrentForm->hasValue("o_fechareal_start") && $this->fechareal_start->CurrentValue != $this->fechareal_start->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_fechasim_start") && $CurrentForm->hasValue("o_fechasim_start") && $this->fechasim_start->CurrentValue != $this->fechasim_start->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_id_actor") && $CurrentForm->hasValue("o_id_actor") && $this->id_actor->CurrentValue != $this->id_actor->OldValue) {
            return false;
        }
    ...
    {
        $this->id_inyect->clearErrorMessage();
        $this->titulo->clearErrorMessage();
        $this->mensaje->clearErrorMessage();
        $this->fechareal_start->clearErrorMessage();
        $this->fechasim_start->clearErrorMessage();
        $this->id_actor->clearErrorMessage();
        $this->para->clearErrorMessage();
        $this->adjunto->clearErrorMessage();
    }
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->id_tareas->CurrentValue = null;
        $this->id_tareas->OldValue = $this->id_tareas->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->id_inyect->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
        ...
        if ($CurrentForm->hasValue("o_mensaje")) {
            $this->mensaje->setOldValue($CurrentForm->getValue("o_mensaje"));
        }
        ...
        if ($CurrentForm->hasValue("o_fechareal_start")) {
            $this->fechareal_start->setOldValue($CurrentForm->getValue("o_fechareal_start"));
        }
        ...
        if ($CurrentForm->hasValue("o_fechasim_start")) {
            $this->fechasim_start->setOldValue($CurrentForm->getValue("o_fechasim_start"));
        }
        ...
        if ($CurrentForm->hasValue("o_id_actor")) {
            $this->id_actor->setOldValue($CurrentForm->getValue("o_id_actor"));
        }
    ...
    protected function newRow()
    {
        ...
        $row['id_tareas'] = $this->id_tareas->CurrentValue;
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->getCurrentMasterTable() == "tareas") {
            $this->id_tareas->CurrentValue = $this->id_tareas->getSessionValue();
        }
        ...
        $masterFilter = str_replace("@id_tarea@", AdjustSql($this->id_tareas->getSessionValue(), "DB"), $masterFilter);
        } else {
            $validMasterRecord = false;
        }
        if ($validMasterRecord) {
        ...
        // id_tareas
        if ($this->id_tareas->getSessionValue() != "") {
            $rsnew['id_tareas'] = $this->id_tareas->getSessionValue();
        }
        if ($this->adjunto->Visible && !$this->adjunto->Upload->KeepFile) {
            $oldFiles = EmptyValue($this->adjunto->Upload->DbValue) ? [] : [$this->adjunto->htmlDecode($this->adjunto->Upload->DbValue)];
            if (!EmptyValue($this->adjunto->Upload->FileName)) {
