 ...
 * Page class
 */
class ArchivosDocAdd extends ArchivosDoc
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ArchivosDocAdd";
        ...
        switch ($this->CurrentAction) {
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "ArchivosDocView") {
    ...
    protected function validateForm()
    {
        ...
        if (in_array("permisos_doc", $detailTblVar) && $detailPage->DetailAdd) {
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->file_name->Visible && !$this->file_name->Upload->KeepFile) {
            $this->file_name->Upload->DbValue = ""; // No need to delete old file
        ...
        if ($this->file_name->Visible && !$this->file_name->Upload->KeepFile) {
            ...
            if (!EmptyValue($this->file_name->Upload->FileName)) {
                ...
                $this->file_name->setDbValueDef($rsnew, $this->file_name->Upload->FileName, null, false);
            }
        }
        ...
        if ($addRow) {
            ...
            $detailPage = Container("PermisosDocGrid");
            if (in_array("permisos_doc", $detailTblVar) && $detailPage->DetailAdd) {
                $detailPage->id_file->setSessionValue($this->id_file->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "permisos_doc"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->id_file->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
