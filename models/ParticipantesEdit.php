 ...
 * Page class
 */
class ParticipantesEdit extends Participantes
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ParticipantesEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("id_participantes") ?? Key(0) ?? Route(2)) !== null) {
                $this->id_participantes->setQueryStringValue($keyValue);
                $this->id_participantes->setOldValue($this->id_participantes->QueryStringValue);
            } elseif (Post("id_participantes") !== null) {
                $this->id_participantes->setFormValue(Post("id_participantes"));
                $this->id_participantes->setOldValue($this->id_participantes->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("id_participantes") ?? Route("id_participantes")) !== null) {
                    $this->id_participantes->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->id_participantes->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$loaded) { // Load record based on key
                    ...
                    $this->terminate("ParticipantesList"); // No matching record, return to list
                    return;
                }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "ParticipantesList") {
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'id_participantes' first before field var 'x_id_participantes'
        $val = $CurrentForm->hasValue("id_participantes") ? $CurrentForm->getValue("id_participantes") : $CurrentForm->getValue("x_id_participantes");
        if (!$this->id_participantes->IsDetailKey) {
            $this->id_participantes->setFormValue($val);
        }
        ...
        // Check field name 'imagen_participante' first before field var 'x_imagen_participante'
        $val = $CurrentForm->hasValue("imagen_participante") ? $CurrentForm->getValue("imagen_participante") : $CurrentForm->getValue("x_imagen_participante");
        if (!$this->imagen_participante->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->imagen_participante->Visible = false; // Disable update for API request
            } else {
                $this->imagen_participante->setFormValue($val);
            }
        }
    ...
    {
        global $CurrentForm;
        $this->id_participantes->CurrentValue = $this->id_participantes->FormValue;
        ...
        $this->imagen_participante->CurrentValue = $this->imagen_participante->FormValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->subgrupo->EditValue = HtmlEncode($this->subgrupo->CurrentValue);
            ...
            $this->imagen_participante->EditValue = HtmlEncode($this->imagen_participante->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->id_participantes->Required) {
            if (!$this->id_participantes->IsDetailKey && EmptyValue($this->id_participantes->FormValue)) {
                $this->id_participantes->addErrorMessage(str_replace("%s", $this->id_participantes->caption(), $this->id_participantes->RequiredErrorMessage));
            }
        }
        ...
        if (!CheckInteger($this->grupo->FormValue)) {
            $this->grupo->addErrorMessage($this->grupo->getErrorMessage(false));
        }
        ...
        if ($this->imagen_participante->Required) {
            if (!$this->imagen_participante->IsDetailKey && EmptyValue($this->imagen_participante->FormValue)) {
                $this->imagen_participante->addErrorMessage(str_replace("%s", $this->imagen_participante->caption(), $this->imagen_participante->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // imagen_participante
            $this->imagen_participante->setDbValueDef($rsnew, $this->imagen_participante->CurrentValue, null, $this->imagen_participante->ReadOnly);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ParticipantesList"), "", $this->TableVar, true);
        $pageId = "edit";
