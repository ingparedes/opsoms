 ...
 * Page class
 */
class SubgrupoList extends Subgrupo
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SubgrupoList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fsubgrupolist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "SubgrupoAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "SubgrupoDelete";
        $this->MultiUpdateUrl = "SubgrupoUpdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fsubgrupolistsrch";
    ...
    public function run()
    {
        ...
        $this->descripcion_subgrupo->Visible = false;
    ...
    {
        if (!SameString($this->id_subgrupo->OldValue, $this->id_subgrupo->CurrentValue)) {
            return false;
        }
        return true;
    }
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->id_subgrupo); // id_subgrupo
            $this->updateSort($this->imagen_subgrupo); // imagen_subgrupo
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->imagen_subgrupo->setSort("");
                $this->nombre_subgrupo->setSort("");
                $this->descripcion_subgrupo->setSort("");
            }
    ...
    public function renderListOptions()
    {
        ...
        if ($this->isInlineEditRow()) { // Inline-Edit
            ...
            $opt->Body .= "<input type=\"hidden\" name=\"k" . $this->RowIndex . "_key\" id=\"k" . $this->RowIndex . "_key\" value=\"" . HtmlEncode($this->id_subgrupo->CurrentValue) . "\">";
            return;
        }
        ...
        $opt->Body = "<div class=\"custom-control custom-checkbox d-inline-block\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"custom-control-input ew-multi-select\" value=\"" . HtmlEncode($this->id_subgrupo->CurrentValue) . "\" onclick=\"ew.clickMultiCheckbox(event);\"><label class=\"custom-control-label\" for=\"key_m_" . $this->RowCount . "\"></label></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fsubgrupolistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fsubgrupolistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fsubgrupolist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->id_subgrupo->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->imagen_subgrupo->UseColorbox) {
                ...
                $this->imagen_subgrupo->LinkAttrs["data-rel"] = "subgrupo_x" . $this->RowCount . "_imagen_subgrupo";
            ...
            if (!EmptyValue($this->imagen_subgrupo->CurrentValue)) {
                if ($this->RowIndex == '$rowindex$') {
                    $this->imagen_subgrupo->Upload->FileName = "";
                } else {
                    $this->imagen_subgrupo->Upload->FileName = $this->imagen_subgrupo->CurrentValue;
                }
            }
            if (is_numeric($this->RowIndex)) {
                RenderUploadField($this->imagen_subgrupo, $this->RowIndex);
            }
    ...
    public function getRowHash(&$rs)
    {
        ...
        $hash = "";
        $hash .= GetFieldHash($row['imagen_subgrupo']); // imagen_subgrupo
        $hash .= GetFieldHash($row['nombre_subgrupo']); // nombre_subgrupo
        return md5($hash);
    }
    ...
    protected function addRow($rsold = null)
    {
        ...
        $masterFilter = str_replace("@id_grupo@", AdjustSql($this->id_grupo->getSessionValue(), "DB"), $masterFilter);
        } else {
            $validMasterRecord = false;
        }
        if ($validMasterRecord) {
