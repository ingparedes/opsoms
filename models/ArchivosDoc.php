...
/**
 * Table class for archivos_doc
 */
class ArchivosDoc extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'archivos_doc';
        $this->TableName = 'archivos_doc';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`archivos_doc`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // id_file
        $this->id_file = new DbField('archivos_doc', 'archivos_doc', 'x_id_file', 'id_file', '`id_file`', '`id_file`', 3, 11, -1, false, '`id_file`', false, false, false, 'FORMATTED TEXT', 'NO');
        $this->id_file->IsAutoIncrement = true; // Autoincrement field
        $this->id_file->IsPrimaryKey = true; // Primary key field
        ...
        // id_users
        $this->id_users = new DbField('archivos_doc', 'archivos_doc', 'x_id_users', 'id_users', '`id_users`', '`id_users`', 3, 11, -1, false, '`id_users`', false, false, false, 'FORMATTED TEXT', 'SELECT');
        ...
        switch ($CurrentLanguage) {
            case "en":
                $this->id_users->Lookup = new Lookup('id_users', 'view_from', false, 'id', ["nombre_agrupacion","","",""], [], [], [], [], [], [], '', '');
                break;
            case "es":
                $this->id_users->Lookup = new Lookup('id_users', 'view_from', false, 'id', ["nombre_agrupacion","","",""], [], [], [], [], [], [], '', '');
                break;
        ...
        // file_name
        $this->file_name = new DbField('archivos_doc', 'archivos_doc', 'x_file_name', 'file_name', '`file_name`', '`file_name`', 200, 100, -1, true, '`file_name`', false, false, false, 'FORMATTED TEXT', 'FILE');
        ...
        // fecha_created
        $this->fecha_created = new DbField('archivos_doc', 'archivos_doc', 'x_fecha_created', 'fecha_created', '`fecha_created`', CastDateFieldForLike("`fecha_created`", 9, "DB"), 135, 19, 9, false, '`fecha_created`', false, false, false, 'FORMATTED TEXT', 'TEXT');
    ...
    public function getDetailUrl()
    {
        // Detail url
        $detailUrl = "";
        if ($this->getCurrentDetailTable() == "permisos_doc") {
            $detailUrl = Container("permisos_doc")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_id_file", $this->id_file->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "ArchivosDocList";
        }
        return $detailUrl;
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`archivos_doc`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->id_file->setDbValue($conn->lastInsertId());
            $rs['id_file'] = $this->id_file->DbValue;
        }
        return $success;
    }
    ...
    {
        // Cascade Update detail table 'permisos_doc'
        $cascadeUpdate = false;
        $rscascade = [];
        if ($rsold && (isset($rs['id_file']) && $rsold['id_file'] != $rs['id_file'])) { // Update detail field 'id_file'
            $cascadeUpdate = true;
            $rscascade['id_file'] = $rs['id_file'];
        }
        if ($cascadeUpdate) {
            $rswrk = Container("permisos_doc")->loadRs("`id_file` = " . QuotedValue($rsold['id_file'], DATATYPE_NUMBER, 'DB'))->fetchAll(\PDO::FETCH_ASSOC);
            foreach ($rswrk as $rsdtlold) {
                ...
                $success = Container("permisos_doc")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("permisos_doc")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("permisos_doc")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }
    ...
    protected function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('id_file', $rs)) {
                AddFilter($where, QuotedName('id_file', $this->Dbid) . '=' . QuotedValue($rs['id_file'], $this->id_file->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        // Cascade delete detail table 'permisos_doc'
        $dtlrows = Container("permisos_doc")->loadRs("`id_file` = " . QuotedValue($rs['id_file'], DATATYPE_NUMBER, "DB"))->fetchAll(\PDO::FETCH_ASSOC);
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("permisos_doc")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("permisos_doc")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("permisos_doc")->rowDeleted($dtlrow);
            }
        }
        if ($success) {
    ...
    public function deleteUploadedFiles($row)
    {
        ...
        $oldFiles = EmptyValue($row['file_name']) ? [] : [$row['file_name']];
        foreach ($oldFiles as $oldFile) {
            if (file_exists($this->file_name->oldPhysicalUploadPath() . $oldFile)) {
    ...
    {
        $keys = [];
        $val = $current ? $this->id_file->CurrentValue : $this->id_file->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->id_file->CurrentValue = $keys[0];
            } else {
                $this->id_file->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('id_file', $row) ? $row['id_file'] : null;
        } else {
            $val = $this->id_file->OldValue !== null ? $this->id_file->OldValue : $this->id_file->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@id_file@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ArchivosDocList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "ArchivosDocEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "ArchivosDocAdd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "ArchivosDocView";
            case Config("API_ADD_ACTION"):
                return "ArchivosDocAdd";
            case Config("API_EDIT_ACTION"):
                return "ArchivosDocEdit";
            case Config("API_DELETE_ACTION"):
                return "ArchivosDocDelete";
            case Config("API_LIST_ACTION"):
                return "ArchivosDocList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ArchivosDocView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("ArchivosDocView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "ArchivosDocAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "ArchivosDocAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ArchivosDocEdit", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("ArchivosDocEdit", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ArchivosDocAdd", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("ArchivosDocAdd", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("ArchivosDocDelete", $this->getUrlParm());
    }
    ...
    {
        $json = "";
        $json .= "id_file:" . JsonEncode($this->id_file->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->id_file->CurrentValue !== null) {
            $url .= "/" . rawurlencode($this->id_file->CurrentValue);
        } else {
...
SORTHTML;
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("id_file") ?? Route("id_file")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->id_file->CurrentValue = $key;
            } else {
                $this->id_file->OldValue = $key;
            }
    ...
    public function getFileData($fldparm, $key, $resize, $width = 0, $height = 0, $plugins = [])
    {
        ...
        $fileNameFld = "";
        $fileTypeFld = "";
        if ($fldparm == 'file_name') {
            $fldName = "file_name";
            $fileNameFld = "file_name";
        } else {
        ...
        if (count($ar) == 1) {
            $this->id_file->CurrentValue = $ar[0];
        } else {
