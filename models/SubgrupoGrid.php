 ...
 * Page class
 */
class SubgrupoGrid extends Subgrupo
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SubgrupoGrid";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fsubgrupogrid";
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->id_subgrupo->CurrentValue;
    ...
    {
        global $CurrentForm;
        if (!EmptyValue($this->imagen_subgrupo->Upload->Value)) {
            return false;
        }
        if ($CurrentForm->hasValue("x_nombre_subgrupo") && $CurrentForm->hasValue("o_nombre_subgrupo") && $this->nombre_subgrupo->CurrentValue != $this->nombre_subgrupo->OldValue) {
            return false;
        }
        return true;
    }
    ...
    {
        $this->id_subgrupo->clearErrorMessage();
        $this->imagen_subgrupo->clearErrorMessage();
        $this->nombre_subgrupo->clearErrorMessage();
    }
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->imagen_subgrupo->Upload->Index = $this->RowIndex;
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_nombre_subgrupo")) {
            $this->nombre_subgrupo->setOldValue($CurrentForm->getValue("o_nombre_subgrupo"));
        }
