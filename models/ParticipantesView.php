 ...
 * Page class
 */
class ParticipantesView extends Participantes
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ParticipantesView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("id_participantes") ?? Route("id_participantes")) !== null) {
            $this->RecKey["id_participantes"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("id_participantes") ?? Route("id_participantes")) !== null) {
                ...
                $this->RecKey["id_participantes"] = $this->id_participantes->QueryStringValue;
            } elseif (Post("id_participantes") !== null) {
                $this->id_participantes->setFormValue(Post("id_participantes"));
                $this->RecKey["id_participantes"] = $this->id_participantes->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->id_participantes->setQueryStringValue($keyValue);
                $this->RecKey["id_participantes"] = $this->id_participantes->QueryStringValue;
            } else {
                $returnUrl = "ParticipantesList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "ParticipantesList"; // No matching record, return to list
                    }
                    break;
            }
        } else {
            $returnUrl = "ParticipantesList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
