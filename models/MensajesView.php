 ...
 * Page class
 */
class MensajesView extends Mensajes
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MensajesView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("id_inyect") ?? Route("id_inyect")) !== null) {
            $this->RecKey["id_inyect"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("id_inyect") ?? Route("id_inyect")) !== null) {
                ...
                $this->RecKey["id_inyect"] = $this->id_inyect->QueryStringValue;
            } elseif (Post("id_inyect") !== null) {
                $this->id_inyect->setFormValue(Post("id_inyect"));
                $this->RecKey["id_inyect"] = $this->id_inyect->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->id_inyect->setQueryStringValue($keyValue);
                $this->RecKey["id_inyect"] = $this->id_inyect->QueryStringValue;
            } else {
                $returnUrl = "MensajesList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "MensajesList"; // No matching record, return to list
                    }
                    break;
            }
        } else {
            $returnUrl = "MensajesList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
