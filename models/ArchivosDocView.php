 ...
 * Page class
 */
class ArchivosDocView extends ArchivosDoc
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ArchivosDocView";
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("id_file") ?? Route("id_file")) !== null) {
                ...
                $this->RecKey["id_file"] = $this->id_file->QueryStringValue;
            } elseif (Post("id_file") !== null) {
                $this->id_file->setFormValue(Post("id_file"));
                $this->RecKey["id_file"] = $this->id_file->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->id_file->setQueryStringValue($keyValue);
                $this->RecKey["id_file"] = $this->id_file->QueryStringValue;
            } else {
                $returnUrl = "ArchivosDocList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "ArchivosDocList"; // No matching record, return to list
                    }
                    break;
            }
        } else {
            $returnUrl = "ArchivosDocList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        // "detail_permisos_doc"
        $item = &$option->add("detail_permisos_doc");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->TablePhrase("permisos_doc", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("PermisosDocList?" . Config("TABLE_SHOW_MASTER") . "=archivos_doc&" . GetForeignKeyUrl("fk_id_file", $this->id_file->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        ...
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'archivos_doc')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=permisos_doc"))) . "\">" . HtmlImageAndText($Language->phrase("MasterDetailViewLink")) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
        ...
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'archivos_doc')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=permisos_doc"))) . "\">" . HtmlImageAndText($Language->phrase("MasterDetailEditLink")) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
        ...
        $item->Body = $body;
        $item->Visible = $Security->allowList(CurrentProjectID() . 'permisos_doc');
        if ($item->Visible) {
            if ($detailTableLink != "") {
                $detailTableLink .= ",";
            }
