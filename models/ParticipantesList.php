 ...
 * Page class
 */
class ParticipantesList extends Participantes
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'participantes';

    // Page object name
    public $PageObjName = "ParticipantesList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fparticipanteslist";
    ...
    public function __construct()
    {
        ...
        // Table object (participantes)
        if (!isset($GLOBALS["participantes"]) || get_class($GLOBALS["participantes"]) == PROJECT_NAMESPACE . "participantes") {
            $GLOBALS["participantes"] = &$this;
        }
        ...
        $this->AddUrl = "ParticipantesAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "ParticipantesDelete";
        $this->MultiUpdateUrl = "ParticipantesUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'participantes');
        }
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fparticipanteslistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("participantes"));
                $doc->Text = @$content;
    ...
    protected function hideFieldsForAddEdit()
    {
        if ($this->isAdd() || $this->isCopy() || $this->isGridAdd()) {
            $this->id_participantes->Visible = false;
        }
    }
    ...
    public function run()
    {
        ...
        $this->id_participantes->setVisibility();
        ...
        $this->imagen_participante->setVisibility();
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->id_participantes->AdvancedSearch->toJson(), ","); // Field id_participantes
        ...
        $filterList = Concat($filterList, $this->imagen_participante->AdvancedSearch->toJson(), ","); // Field imagen_participante
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fparticipanteslistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field id_participantes
        $this->id_participantes->AdvancedSearch->SearchValue = @$filter["x_id_participantes"];
        $this->id_participantes->AdvancedSearch->SearchOperator = @$filter["z_id_participantes"];
        $this->id_participantes->AdvancedSearch->SearchCondition = @$filter["v_id_participantes"];
        $this->id_participantes->AdvancedSearch->SearchValue2 = @$filter["y_id_participantes"];
        $this->id_participantes->AdvancedSearch->SearchOperator2 = @$filter["w_id_participantes"];
        $this->id_participantes->AdvancedSearch->save();
        ...
        // Field imagen_participante
        $this->imagen_participante->AdvancedSearch->SearchValue = @$filter["x_imagen_participante"];
        $this->imagen_participante->AdvancedSearch->SearchOperator = @$filter["z_imagen_participante"];
        $this->imagen_participante->AdvancedSearch->SearchCondition = @$filter["v_imagen_participante"];
        $this->imagen_participante->AdvancedSearch->SearchValue2 = @$filter["y_imagen_participante"];
        $this->imagen_participante->AdvancedSearch->SearchOperator2 = @$filter["w_imagen_participante"];
        $this->imagen_participante->AdvancedSearch->save();
    ...
    protected function basicSearchSql($arKeywords, $type)
    {
        ...
        $this->buildBasicSearchSql($where, $this->subgrupo, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->imagen_participante, $arKeywords, $type);
        return $where;
    }
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->id_participantes); // id_participantes
            ...
            $this->updateSort($this->imagen_participante); // imagen_participante
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->imagen_participante->setSort("");
    ...
    public function renderListOptions()
    {
        ...
        $opt->Body = "<div class=\"custom-control custom-checkbox d-inline-block\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"custom-control-input ew-multi-select\" value=\"" . HtmlEncode($this->id_participantes->CurrentValue) . "\" onclick=\"ew.clickMultiCheckbox(event);\"><label class=\"custom-control-label\" for=\"key_m_" . $this->RowCount . "\"></label></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fparticipanteslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fparticipanteslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fparticipanteslist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchSortOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fparticipanteslistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
